...
void HostBase::sendMsg(cMessage* msg) {
    sendMsg(msg, "udplink", cGate::INOUT);
}

void HostBase::sendMsg(cMessage* msg, const char* link, cGate::Type linkType) {
    if(linkType == cGate::Type::INOUT){
        send(msg, this->gateHalf(link, cGate::OUTPUT));
    }else{
        send(msg, this->gate(link));
    }
}
...
IPvXAddress HostBase::loadAddress(const char* interface){
    ...
    IPvXAddress ipddress(ipData->getIPAddress());
    return ipddress;
}

UDPControlInfo* HostBase::generateControlInfo(IPvXAddress src, IPvXAddress dest){
    UDPControlInfo* udpControlInfo = new UDPControlInfo();
    udpControlInfo->setDestAddr(dest);
    udpControlInfo->setSrcAddr(src);
    return udpControlInfo;
}

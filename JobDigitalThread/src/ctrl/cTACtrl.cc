...
void cTACtrl::initialize(int stage) {
    if (stage == 0) {
        ...
        pull_threshold = getParentModule()->par("pull_threshold");
        assoc_accuracy = getParentModule()->par("assoc_accuracy");
    }
...
void cTACtrl::dispatchHandler(cMessage *msg) {
    if (msg->isName(msg::REMOTE_UPDATE)) {
        ...
        delete msg;
    } else if (msg->isName(msg::LAST_UPDATE)) {
        LastUpdate *update = check_and_cast<LastUpdate*>(msg);
        handleLastUpdate(update);
        delete msg;
    }
}
...
void cTACtrl::handleRemoteUpdate(RemoteUpdate *update) {
        ...
        for (auto &it2 : it.second) {
            ...
            double p_pull =
                    KnowledgeBaseAccess().get(getParentModule())->queryPullRate(
                            dt_index, var);
            if (p_pull >= pull_threshold) {    // association accuracy = 100%
                if (uniform(0, 1) < assoc_accuracy) {  // association accuracy = 50%
    ...
    if (lost["lost_digest"].size() > 0 && !isPull) {
        PullRequest *pull = new PullRequest(msg::PULL_REQUEST);
        pull->setRequest(lost.dump().c_str());
        pull->setRound(round);
        pull->setTs(simTime());
        IPvXAddress dest = loadAddress("eTAAddress");
        pull->setControlInfo(this->generateControlInfo(ipAddress, dest));
        sendIdeal(pull);

        // for communication overhead analysis
        ...
        for (auto &it : dtLastUpdate) {
            ...
            dtUpdate->setControlInfo(
                    this->generateControlInfo(ipAddress, dest));
            sendIdeal(dtUpdate);
        }
        ...
        for (auto &it : l_digest.items()) {
            ...
            for (auto &it2 : dtDigest.items()) {
                ...
                string dtKey = dt + "-" + key;
                GlobalStatisticsAccess().get()->addCloudDigest(dtKey,
                        frequency);
            }
        }
...
void cTACtrl::handleLastUpdate(LastUpdate *update) {
    json updates = json::parse(update->getUpdates());
    int count = update->getCount();
    json r_digest = json::parse(update->getDigest());
    int granularity = update->getGranularity();
    ...
    EV << "last updates: " << updates << "\n";
    EV << "last count: " << count << "\n";
    EV << "last granularity: " << granularity << "\n";
    EV << "last digest: " << r_digest << "\n";
    ...
    for (auto &it : updates.items()) {
        string first = it.key();
        int msg_index = util::strToInt(first);
        last_receivedIndices.insert(msg_index);
        json j_update = it.value();
        int dt_index = j_update["dt_index"].get<int>();
        int u_index = j_update["u_index"].get<int>();
        last_cache[dt_index][u_index] = j_update;
    }

    // search missing messages
    for (int i = 0; i < count; i++) {
        if (last_receivedIndices.count(i) == 0) {
            lost["indices"].push_back(i);
        }
    }
    ...
    if (lost["indices"].size() > 0) {
        PullRequest *pull = new PullRequest(msg::LAST_PULL_REQUEST);
        pull->setRequest(lost.dump().c_str());
        pull->setRound(round);
        pull->setTs(simTime());
        IPvXAddress dest = loadAddress("eTAAddress");
        ...
        for (auto &it : last_cache) {
            ...
            for (auto &it2 : it.second) {
                json dtUpdate = it2.second;
                string key = util::intToStr(dtUpdate["key"].get<int>());
                dtLastUpdate[dtIndex][key] = dtUpdate;
            }
        }
        ...
        for (auto &it : dtLastUpdate) {
            string dt = "cDT[" + util::intToStr(it.first) + "]";
            json update = it.second;
            cDTLastUpdate *dtUpdate = new cDTLastUpdate(msg::cDT_LAST_UPDATE);
            dtUpdate->setUpdates(update.dump().c_str());
            dtUpdate->setRound(round);
            dtUpdate->setTs(simTime());
            IPvXAddress dest = loadAddress(dt.c_str());

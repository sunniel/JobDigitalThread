...
void cDACtrl::initialize(int stage) {
    if (stage == 0) {
        ...
        var_num = getSimulation()->getSystemModule()->par("var_num");
...
void cDACtrl::dispatchHandler(cMessage *msg) {
    if (msg->isName(msg::cDT_UPDATE)) {
        ...
        delete msg;
    } else if (msg->isName(msg::cDT_LAST_UPDATE)) {
...
void cDACtrl::handleDTLastUpdate(cDTLastUpdate *update) {
    ...
    if (uniform(0, p_pull) >= pull_threshold) {
//    if (p_pull >= pull_threshold) {

//        cout << fullName << " sim-pull" << endl;

        SimPull *pull = new SimPull(msg::SIM_PULL);
        pull->setDt(index);
        IPvXAddress dest = this->loadAddress("eTAAddress");
        pull->setRound(roundIndex);
...
//    int roundIndex = update->getRound();
//    // simulate-pull determination by checking sample length
//    double p_var = p_target;
////    double p_pull = 1 - pow((1 - p_var), sample);
//
//    double p_pull = 0;
//    if(sigma != 0){
//        double p_pull_1 = 1 - 1.5 / (pow(pull_radix, pull_beta) * log(100 * sigma));
//        p_pull = 1 - pow((1 - p_pull_1), sample);
////        p_pull = p_pull_1;
//    }
//
//    p_pull = p_pull > 1  ? 1 : p_pull;
//
//    if (uniform(0, p_pull) >= pull_threshold) {
//
//        cout << fullName << " sim-pull" << endl;
...
//    }
//
//    sigma = exp(1.5 / (pow(pull_radix, pull_beta) * (1 - p_var))) / (100 * var_num);
//    sigma += (sample - 1) * pull_beta * sigma;
//    for (auto &it : state) {
//        it.second = it.second + (1 + sigma) * (round / cycle - 1);
//    }
//
